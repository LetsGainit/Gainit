...
{
    public class ProjectService2 : IProjectService
    {
        private readonly GainItDbContext r_DbContext;
        public ProjectService2(GainItDbContext i_DbContext)
        {
        ...
        private async Task<UserProject> loadFullProjectAsync(Guid projectId)
        {
            return await r_DbContext.Projects
                .Include(p => p.TeamMembers) // Changed from ProjectTeamMembers to TeamMembers
        ...
        {
            // Refactored: Using loadFullProjectAsync to avoid duplicating Include statements
        ...
        public async Task<UserProjectViewModel> AssignMentorAsync(Guid i_ProjectId, Guid i_MentorId)
        {
            ...
            // Refactored: Using loadFullProjectAsync to get full project data
            return new UserProjectViewModel(await r_DbContext.Projects
                .Include(p => p.AssignedMentor)
                .FirstOrDefaultAsync(p => p.ProjectId == i_ProjectId));
        }
        ...
        public async Task<UserProjectViewModel> AddTeamMemberAsync(Guid i_ProjectId, Guid i_UserId)
        {
            ...
            if (o_Project.ProjectMembers.Any(u => u.UserId == i_UserId)) // Check if the user is already a team member
            {
            ...
            {
                UserId = o_Gainer.UserId,
                Project = o_Project,
                Role = eProjectRole.Member,
                JoinDate = DateTime.UtcNow
            };
            o_Project.ProjectMembers.Add(projectMember);
            await r_DbContext.SaveChangesAsync();
        ...
        public async Task<UserProjectViewModel> RemoveTeamMemberAsync(Guid i_ProjectId, Guid i_UserId)
        {
            ...
            var projectMember = o_Project.ProjectMembers.FirstOrDefault(pm => pm.UserId == i_UserId);
            if (projectMember == null)
            {
                throw new KeyNotFoundException("User not found in project.");
            }
            o_Project.ProjectMembers.Remove(projectMember);
            await r_DbContext.SaveChangesAsync();
        ...
        public async Task<UserProjectViewModel> StartProjectFromTemplateAsync(Guid i_TemplateId, Guid i_UserId)
        {
            ...
            UserProject o_newProject = new UserProject
            {
                ...
                TeamMembers = new List<Gainer> { Gainer }, // Changed from ProjectTeamMembers to TeamMembers
            };
        ...
        public async Task<UserProjectViewModel> CreateProjectForNonprofitAsync(UserProjectViewModel i_Project, Guid i_NonprofitOrgId)
        {
            ...
            UserProject o_NonprofitNewProject = new UserProject
            {
                ...
                TeamMembers = new List<Gainer>(), // Changed from ProjectTeamMembers to TeamMembers
            };
